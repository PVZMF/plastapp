...
import {deleteProduct, discountCreate, discountDelete, discountUpdate, getListMyProduct } from "../../../api/api";
            ...
            <tbody>
              {listProducts?.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>
                    <Button variant="contained" sx={{width:"100%"}} color="error" onClick={()=>{handleClickOpen(item.id,item.title)}}>حذف محصول</Button>
                  </td>
                  <td>
...
<div>
      ...
      >
        <DialogTitle sx={{minWidth:"300px" ,fontSize:"20px",width:"100%"}}>آیا از حذف محصول <span style={{color:"#3C76D2"}}>{deleteState.title}</span> مطمئن  هستید؟
</DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          <Button variant="contained" color="error"onClick={handleClose} sx={{marginLeft:"20px"}}>خیر</Button>
          <Button variant="contained" onClick={()=>deleteProductHandler(deleteState.id)}>بله</Button>
        </DialogActions>
      </Dialog>
    </div>
    <ToastContainer/>
