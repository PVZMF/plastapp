    ...
    <div className={style.address}>
      <div className={style.card}>
        <div className={style.header}>
          <div className={style.title}>
            <button onClick={() => dispatch(stepMinus())}>
              <BsArrowRight /> انتخاب آدرس
            </button>
          </div>
        </div>
        ...
        <div className={style.data_address}>
          ...
          <div className={style.footerCard}>
            <h3 style={{ color: "black" }}>
              کد پستی: {toPersianNumber(state.sendInfo.postal_code)}
            </h3>
            <h3 style={{ color: "black" }}>
              شماره‌همراه: {toPersianNumber(state.sendInfo.phone_number)}
            </h3>
          </div>
        </div>
      </div>
      ...
      <div className={style.card}>
        <div className={style.header}>
          <div className={style.title}>
            <button>
              هزینه ارسال برای {state.selectedItems.length} بسته{" "}
              {state.sendPrice > 0
                ? `${toPersianNumber(state.sendPrice)} تومان`
                : "رایگان"}
            </button>
          </div>
        </div>
        ...
        <div className={style.boxCards}>
          {state.selectedItems.map((item, index) => (
            <Card
              ...
              index={index}
              item={item}
              all={state.selectedItems.length}
            />
          ))}
        </div>
      </div>
